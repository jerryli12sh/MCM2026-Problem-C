...
def build_base(df_roster: pd.DataFrame, elim_long: pd.DataFrame, df_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    # optional enrich from df_clean
    extra_cols = ["celebrity_industry", "celebrity_homestate", "celebrity_homecountry_region"]
    if df_clean is not None and "celebrity_name" in df_clean.columns:
        have_cols = [c for c in extra_cols if c in df_clean.columns]
        if have_cols:
            if "season" in df_clean.columns and "season" in base.columns:
                key_cols = ["season", "celebrity_name"]
            else:
                key_cols = ["celebrity_name"]
            extra = (
                df_clean[key_cols + have_cols]
                .drop_duplicates(subset=key_cols)
                .copy()
            )
            base = base.merge(extra, on=key_cols, how="left")
...
def build_panel(base: pd.DataFrame, judge_percent: pd.DataFrame, judge_rank_share: pd.DataFrame,
    ...
    panel = add_judge_momentum(panel, window=3)
    return panel


def add_judge_momentum(panel: pd.DataFrame, window: int = 3) -> pd.DataFrame:
    out = panel.copy()
    out["j_metric"] = pd.to_numeric(out["j_metric"], errors="coerce")
    out["j_momentum"] = 0.0

    def slope_for_series(values):
        n = len(values)
        if n < 2:
            return 0.0
    ...
    def compute_group(g):
        ...
        vals = g["j_metric"].to_numpy()
        slopes = []
        for i in range(len(vals)):
            start = max(0, i - window + 1)
            y = vals[start:i + 1]
            slopes.append(slope_for_series(y))
        g["j_momentum"] = slopes
        return g

    out = (
        out.groupby(["season", "celebrity_name"], group_keys=False)
        .apply(compute_group)
    )
    return out
...
def _build_features(panel: pd.DataFrame, train_rows: pd.DataFrame):
    ...
    df_feat["j_momentum"] = pd.to_numeric(df_feat.get("j_momentum", 0.0), errors="coerce").fillna(0.0)
    mom_mean = df_feat["j_momentum"].mean()
    mom_std = df_feat["j_momentum"].std(ddof=0) + 1e-12
    df_feat["j_momentum_z"] = (df_feat["j_momentum"] - mom_mean) / mom_std

    # industry one-hot
    if "celebrity_industry" in df_feat.columns:
        industry_series = df_feat["celebrity_industry"].fillna("Unknown").astype(str)
    else:
        industry_series = pd.Series(["Unknown"] * len(df_feat))
    industry_levels = sorted(industry_series.unique().tolist())
    industry_dummies = pd.get_dummies(industry_series, prefix="industry")
    df_feat = pd.concat([df_feat, industry_dummies], axis=1)

    # homestate label encoding
    if "celebrity_homestate" in df_feat.columns:
        homestate_series = df_feat["celebrity_homestate"].fillna("Unknown").astype(str)
    else:
        homestate_series = pd.Series(["Unknown"] * len(df_feat))
    homestate_levels = sorted(homestate_series.unique().tolist())
    homestate_map = {k: i for i, k in enumerate(homestate_levels)}
    df_feat["homestate_code"] = homestate_series.map(homestate_map).astype(int)

    # homecountry region binary: United States -> 0, others -> 1
    if "celebrity_homecountry_region" in df_feat.columns:
        region_series = df_feat["celebrity_homecountry_region"].fillna("Unknown").astype(str)
    else:
        region_series = pd.Series(["Unknown"] * len(df_feat))
    df_feat["homecountry_is_non_us"] = (region_series != "United States").astype(float)
    ...
    industry_cols = [c for c in df_feat.columns if c.startswith("industry_")]
    X_cols = ["j_metric_z", "j_momentum_z", "age_z", "era_is_percent", "homestate_code", "homecountry_is_non_us"] + industry_cols
...
def build_features_for_rows(df_rows: pd.DataFrame, pooled_fit: dict) -> pd.DataFrame:
    ...
    out["j_momentum"] = pd.to_numeric(out.get("j_momentum", 0.0), errors="coerce").fillna(0.0)
    mom_mean = pooled_fit.get("mom_mean", 0.0)
    mom_std = pooled_fit.get("mom_std", 1.0)
    out["j_momentum_z"] = (out["j_momentum"] - mom_mean) / (mom_std + 1e-12)
    ...
    # industry one-hot using training levels
    industry_levels = pooled_fit.get("industry_levels", ["Unknown"])
    if "celebrity_industry" in out.columns:
        industry_series = out["celebrity_industry"].fillna("Unknown").astype(str)
    else:
        industry_series = pd.Series(["Unknown"] * len(out))
    industry_series = industry_series.where(industry_series.isin(industry_levels), "Unknown")
    industry_dummies = pd.get_dummies(industry_series, prefix="industry")
    for level in industry_levels:
        col = f"industry_{level}"
        if col not in industry_dummies.columns:
            industry_dummies[col] = 0
    out = pd.concat([out, industry_dummies], axis=1)

    # homestate label encoding using training map
    homestate_map = pooled_fit.get("homestate_map", {"Unknown": 0})
    if "celebrity_homestate" in out.columns:
        homestate_series = out["celebrity_homestate"].fillna("Unknown").astype(str)
    else:
        homestate_series = pd.Series(["Unknown"] * len(out))
    homestate_series = homestate_series.where(homestate_series.isin(homestate_map.keys()), "Unknown")
    out["homestate_code"] = homestate_series.map(homestate_map).fillna(homestate_map.get("Unknown", 0)).astype(int)

    # homecountry region binary: United States -> 0, others -> 1
    if "celebrity_homecountry_region" in out.columns:
        region_series = out["celebrity_homecountry_region"].fillna("Unknown").astype(str)
    else:
        region_series = pd.Series(["Unknown"] * len(out))
    out["homecountry_is_non_us"] = (region_series != "United States").astype(float)
    ...
    for col in pooled_fit.get("X_cols", []):
