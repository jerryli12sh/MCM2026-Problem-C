...
legacy producer, expected value or visual target, a registered tolerance, and status.

Tolerances are registered release defaults; changing one requires an explicit decision.
Numeric tolerances are relative where the pipeline is deterministic;
...
def main() -> int:
    ...
    lines = [
        ...
        "> These are the registered release tolerances used by the automated comparison.",
